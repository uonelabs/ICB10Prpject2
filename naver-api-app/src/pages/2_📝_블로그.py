...
from naver_api import NaverAPIError, search_blog

st.set_page_config(page_title="블로그 검색", page_icon="📝", layout="wide")
client_id, client_secret = render_sidebar()

st.title("📝 블로그 검색")
st.caption("검색어별 블로그 검색 결과 수를 비교하고, 게시물 목록을 확인합니다.")

require_credentials(client_id, client_secret)

col1, col2, col3 = st.columns([2, 1, 1])
with col1:
    keywords_raw = st.text_input("검색어 (쉼표로 구분, 최대 5개)", placeholder="맛집, 카페, 여행")
with col2:
    display = st.slider("검색어당 결과 수", 10, 100, 20, step=10)
with col3:
    sort = st.selectbox("정렬", ["sim", "date"], format_func=lambda x: {"sim": "정확도순", "date": "날짜순"}[x])

use_date_filter = st.checkbox("작성일로 결과 필터링")
if use_date_filter:
    default_end = date.today()
    default_start = default_end - timedelta(days=365)
    start_date, end_date = st.date_input("작성일 범위", value=(default_start, default_end), max_value=default_end)
...
if st.button("조회", type="primary"):
    ...
    try:
        with st.spinner("블로그 검색 중..."):
            for kw in keywords:
                result = search_blog(client_id, client_secret, kw, display=display, sort=sort)
    ...
    for tab, kw in zip(tabs, keywords):
        with tab:
            ...
            if use_date_filter and "postdate" in df.columns:
                df["postdate_dt"] = pd.to_datetime(df["postdate"], format="%Y%m%d", errors="coerce")
                mask = (df["postdate_dt"].dt.date >= start_date) & (df["postdate_dt"].dt.date <= end_date)
                df = df[mask].drop(columns=["postdate_dt"])
            cols = [c for c in ["title", "bloggername", "postdate", "link", "description"] if c in df.columns]
