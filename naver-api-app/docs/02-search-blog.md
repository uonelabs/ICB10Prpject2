...
```
GET https://openapi.naver.com/v1/search/blog.json?query=검색어&display=10&start=1&sort=sim
```
...
| items[].title | String | 블로그 포스트 제목 (검색어 일치 부분은 `<b>` 태그로 강조) |
| items[].link | String | 블로그 포스트 URL |
| items[].description | String | 포스트 내용 요약 (검색어 일치 부분은 `<b>` 태그로 강조) |
| items[].bloggername | String | 블로그 이름 |
| items[].bloggerlink | String | 블로그 주소 |
| items[].postdate | String | 포스트 작성 날짜 (yyyyMMdd) |
...
```json
{
  ...
  "items": [
    {
      "title": "예시 <b>검색어</b> 포스트 제목",
      "link": "https://blog.example.com/post/1",
      "description": "포스트 내용 요약에서 <b>검색어</b>와 일치하는 부분",
      "bloggername": "예시 블로그",
